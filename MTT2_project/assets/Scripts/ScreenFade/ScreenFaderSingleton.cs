...
public class ScreenFaderSingleton : Singleton<ScreenFaderSingleton> {
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Escape))
        {
            ...
            {
                if (!GameObject.Find("Manager").GetComponent<Setup>().CheckIfGameStarted() || Time.timeScale != 1 || GameObject.Find("Vehicles").GetComponentInChildren<TrainMovement>().CheckIfGameEnded())
                    FadeAndGoBack();
            }
            //if (SceneManager.GetActiveScene().buildIndex == 1)
            //{
        ...
        #region music menu from other game
    ...
    public void FadeAndGoToHomeScreen()
    {
        Invoke("FadeOut", 0f);
        Invoke("LoadHomeScreen", 1.25f);
        Invoke("FadeIn", 2f);
    }
